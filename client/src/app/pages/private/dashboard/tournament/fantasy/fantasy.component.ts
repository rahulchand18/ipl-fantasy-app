...
export class FantasyComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private matchService: MatchService, private snackbar: MatSnackBar, private authService: AuthService) {
  ...
  saveTeam() {
    ...
    for (const team in teamCounts) {
      if (teamCounts[team] > 6) {
        this.snackbar.open(`You cannot select more than 6 players from ${team}!`, 'Close', {
          duration: 3000
        });
        return;
      }
    ...
    let captainToUpdate = this.selectedPlayers.find(player => player.id === this.captain);
    if (captainToUpdate) {
      captainToUpdate.isCaptain = true;
      captainUpdated = true
    }

    let viceCaptainToUpdate = this.selectedPlayers.find(player => player.id === this.viceCaptain);
    ...
    if (!captainUpdated || !viceCaptainUpdated) {
      this.snackbar.open('Please select one captain and one vice-captain', 'Close', {
        duration: 3000
      })
      return
    }
  ...
  getAllPredictionsByMatch() {
    this.matchService.getOnePrediction(this.matchId, this.loginUser.email).subscribe(res => {
