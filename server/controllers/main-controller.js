    ...
    for(const match of activeMatches){
      const summary = await Scorecard.aggregate([
        ...
        },
        {
          $group: {
            _id: null,
            mostRuns: { $max: { runs: "$players.runs", name: "$players.name" } },
            mostWickets: { $max: { wickets: "$players.wickets", name: "$players.name" } },
            mostCatches: { $max: { catch: "$players.catch", name: "$players.name" } },
            mostSixes: { $max: { sixes: "$players.sixes", name: "$players.name" } },
            t1s: { $first: "$t1s" },
            t2s: { $first: "$t2s" },
            status: { $first: "$status" },
          },
